...
// 카운터 강조 시작 지점 — 한도에 가까워졌을 때만 시선을 끈다.
  ...
  return (
    <SafeAreaView edges={['top']} style={styles.container}>
      ...
      >
        ...
        <View
          className={`mt-md w-full flex-col gap-xs rounded-lg border bg-white px-lg py-md shadow-sm ${
        ...
        >
          ...
          <Typography
            ...
            className={`w-full text-right ${
              query.length >= COUNTER_WARN_AT ? 'font-bold text-warning-700' : 'text-text-tertiary'
            }`}
          >
        ...
        <Typography variant="caption" className="mt-md">
          이렇게 입력해 보세요
        </Typography>
        <View className="w-full flex-row flex-wrap gap-sm">
          {EXAMPLES.map((example) => (
            <Pressable
              ...
              className="rounded-full border border-gray-200 bg-white px-md py-sm active:bg-primary-50"
